...
const artist0 = artists[0].name;


//(2) Bio of the third artist (2nd index) in the array 
const bio2 = artists[2].bio
...
function getArtistByIndex(artists, index) {
  const getIndexString = artists[0].name;
 // const getIndexString = artists[0] =`the artist at index ${"id"} is ${"name"}`
...
function listOfNames(artists) {
  const copyOfArtistArray = [...artists]
  for(let i = 0; i < copyOfArtistArray.length; i++){
    copyOfArtistArray[i] = copyOfArtistArray[i].name
  }
  return copyOfArtistArray
}
  // console.log(listOfNames(arrayArtists))
...
function removeArtist(artist, index) {
  const copyOfArtistArray = [...artists]
  copyOfArtistArray.splice(index, 1)
  return copyOfArtistArray
}
...
function addArtist(artist, name, years, genre, nationality, bio) {
  ...
  const artistToAdd =
  {
    name: name, years: years, genre: genre, nationality: nationality, bio: bio
  }
  copyOfArtistArray.push(artistToAdd)
  return copyOfArtistArray
}
...
function lotsOfArt(artists) {
  ...
  for(let i = 0; i < artists.length; i++) {
    if(artists[i].paintings > 100) {
      arrayToReturn.push(artists[i].name)
    }
  }
